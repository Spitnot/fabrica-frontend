...
async function getOrders() {
  const { data } = await supabaseAdmin
    .from('orders')
    .select('id, status, total_productos, peso_total, created_at, customer:customers(contacto_nombre, company_name)')
              ...
              {orders.length === 0 ? (
                ...
                orders.map((o: any) => (
                  <tr key={o.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <Link href={`/pedidos/${o.id}`} className="text-xs text-[#D93A35] font-semibold hover:text-[#b52e2a]">
                        View →
                      </Link>
                    </td>
                  </tr>
