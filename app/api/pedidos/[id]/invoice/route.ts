...
function getTaxLogic(customer: any, company: any) {
  ...
  if (isSpain) {
    ...
    return { rate, label: `IVA (${rate}%)`, mention: null }
  }
  ...
  return { rate: 0, label: 'VAT (0%)', mention: 'Exento de IVA — Exportación (art. 21 LIVA)' }
}

export async function GET(_req: NextRequest, { params }: Props) {
  // Auth: staff can see any invoice; customers can only see their own
  ...
  const isStaff = ['admin', 'manager', 'viewer'].includes(role)
  if (!isStaff) {
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 11px; color: #111; background: #fff; padding: 32px 40px; max-width: 820px; margin: 0 auto; }
    .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #111; padding-bottom: 18px; margin-bottom: 22px; }
    ...
    .addresses { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 22px; padding: 16px; background: #f9f9f9; border: 1px solid #eee; }
    .address-block h3 { font-size: 8px; font-weight: 800; letter-spacing: .2em; text-transform: uppercase; color: #999; margin-bottom: 8px; padding-bottom: 4px; border-bottom: 1px solid #eee; }
    ...
    .incoterm-bar { display: flex; gap: 16px; padding: 8px 16px; margin-bottom: 18px; background: #fff; border: 1px solid #eee; font-size: 10px; color: #666; }
    .incoterm-bar span { font-weight: 700; color: #111; }
    table { width: 100%; border-collapse: collapse; }
    thead tr { background: #111; color: #fff; }
    thead th { padding: 9px 10px; text-align: left; font-size: 8px; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; }
    ...
    .totals-row { display: flex; justify-content: space-between; padding: 5px 0; font-size: 11px; color: #555; border-bottom: 1px solid #f0f0f0; }
    .totals-row.tax { color: #777; }
    .totals-row.grand { border-bottom: none; border-top: 2px solid #111; margin-top: 6px; padding-top: 10px; font-weight: 800; font-size: 14px; color: #111; }
    .totals-row.grand span:last-child { color: ${primaryColor}; }
    .mention { margin-top: 18px; padding: 10px 14px; background: #fffbf0; border-left: 3px solid #E6883E; font-size: 10px; color: #555; line-height: 1.6; }
    .payment-info { margin-top: 18px; padding: 12px 16px; border: 1px solid #eee; background: #f9f9f9; }
    .payment-info h4 { font-size: 8px; font-weight: 800; letter-spacing: .18em; text-transform: uppercase; color: #999; margin-bottom: 8px; }
    ...
    .footer { margin-top: 32px; padding-top: 12px; border-top: 1px solid #e5e5e5; font-size: 9px; color: #bbb; display: flex; justify-content: space-between; }
...
<body>
  <div class="no-print" style="margin-bottom:20px;text-align:right;display:flex;gap:10px;justify-content:flex-end">
    <button onclick="window.print()" style="padding:8px 18px;background:#111;color:#fff;border:none;font-size:12px;font-weight:600;cursor:pointer;letter-spacing:.04em">🖨 Print / Save PDF</button>
  </div>
  <div class="header">
    <div>
  ...
  <table>
    <thead>
      <tr>
        <th>Producto</th><th>SKU</th><th class="center">Qty</th>
        <th class="right">Peso</th><th class="right">Precio unit.</th><th class="right">Total</th>
      </tr>
    </thead>
  ...
  <div class="totals">
    <div class="totals-inner">
      ...
      <div class="totals-row tax"><span>${tax.label}</span><span>${tax.rate === 0 ? '0,00 €' : fmt(taxAmount)}</span></div>
      ${order.coste_envio_final ? `<div class="totals-row"><span>Envío (${incoterm})</span><span>${fmt(order.coste_envio_final)}</span></div>` : ''}
