...
export default function ForgotPasswordPage() {
  ...
  const handleReset = async () => {
    ...
    try {
      // 1. Call backend API to generate a reset link securely
      ...
      // We proceed even if the API returns an error (generic message for security)
      // But if successful, we send the email.
      if (res.ok && data.link) {
        ...
        const htmlContent = `
          <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: auto;">
            ...
            <p style="font-size: 12px; color: #999;">If you did not request this, please ignore this email. This link will expire in 24 hours.</p>
          </div>
        `;

        await sendEmail(email, 'Reset Your Fabrica Password', htmlContent);
      }
      ...
      <div className="w-full max-w-sm">
        ...
        {message ? (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center text-sm text-green-800">
            {message}
            <div className="mt-4">
              <Link href="/login" className="text-xs text-[#D93A35] font-semibold hover:underline">
                ← Back to Login
              </Link>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-[#D93A35] outline-none transition-colors"
              />
            </div>
